...
static inline bool CharToWString(LPCSTR str, std::wstring& out_wstr)
{
    int length = MultiByteToWideChar(CP_UTF8, 0, str, -1, nullptr, 0);
    if (length > 0)
    {
        out_wstr.resize(length - 1);
        if (MultiByteToWideChar(CP_UTF8, 0, str, -1, &out_wstr[0], length) > 0)
            return true;
    }
...
Key Key::OpenRootKey(void* hRoot, const char* szSubKey, uint32_t rights, bool create)
{
    std::wstring wstr;
    Key k;
    if (CharToWString(szSubKey, wstr))
